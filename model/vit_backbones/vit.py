...
from model.configs_vpt import vit_configs as configs
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, vis=False):
        ...
        # custom output
        cls_token = x[:, 0]
        patch_token = self.avg_pool(x[:, 1:].transpose(1, 2)).squeeze(-1)
        return torch.cat([cls_token.unsqueeze(1), patch_token.unsqueeze(1)], dim = 1)
